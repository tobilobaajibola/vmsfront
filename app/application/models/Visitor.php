...
class Visitor extends CI_Model
{
	...
	{
		return  $this->db->query("select *, visitor.firstname as firstname, visitor.lastname as lastname, visitor.email as visitor_email, visitor.capture_name as capture_name, visitor.phone as visitor_phone, employee.firstname as employee_firstname, employee.lastname as employee_lastname, employee.email as employee_email, employee.phone as employee_phone, checkin.status as checkin_status   from checkin left join visitor on (checkin.visitor_id = visitor.id) left join employee on (checkin.employee_id = employee.id) where (checkin.id = '$checkin_id')")->row_array();

	}

	function get_visitor_detail($visitor_id){
		return $this->db->get_where('visitor', array('id'=>$visitor_id))->row_array();
	}
	...
	function checkout_visitor($checkout){
		...
		$this->db->update('checkin',  array('check_out_time' => $checkout['checkout_time'], 'check_out_date' => $checkout['checkout_date'], 'checkout_desk'=>$checkout['checkout_desk'], 'status' => $checkout['status'],'comment' =>$checkout['comment']  ));
	...
	function verify_vistor_pre_visit($email, $phone, $company_id){
		...
		$query_verify_vistor_pre_visit = $this->db->get_where('visitor', array('company_id'=>$company_id));

			return $query_verify_vistor_pre_visit->row_array();
	}


	function get_visitor_tag($company_id, $desk_id, $purpose){
		// visitor tag with empty purpose id will be assigned by default while the ones with purpose id will be assigned to the respective purpose

	// return  $this->db->query("select * from visitor_tag where(company_id = $company_id and desk_id_out = $desk_id and (case when purpose != 0 then purpose = '$purpose' and status = 0 else  status = 0  end))")->row_array();
	...
	function get_all_visitor_tag($company_id){
		return  $this->db->query("select *, visitor.firstname as visitor_firstname, visitor.lastname as visitor_lastname, visitor_tag.status as status, visitor_tag.id as tag_id, (select desk_name from company where company.company_id = $company_id and visitor_tag.desk_id_out = company.id)desk_out_name,
		 (select desk_name from company where company.company_id = $company_id and visitor_tag.desk_id_in = company.id)desk_in_name   from visitor_tag left join visitor on (visitor_tag.last_checkin_visitor_id = visitor.id) 
			where (visitor_tag.company_id = $company_id)")->result_array();
	}
    ...
    function get_list_of_checkin($company_id, $no_of_checkin, $location_id='', $start_date ='',  $end_date='')
    {
	...
	where (c.visitor_id = v.id  and c.company_id = $company_id $location   and c.check_in_date >='$start_date' and c.check_in_date <= '$end_date')  order by c.id desc LIMIT $no_of_checkin") ;
