		...
		<div class="card <?php 	if($checkin_data['blacklist_status']== 1){echo 'text-danger';}?>">
					 ...
					 -->
						<?php if(!empty($checkin_data['capture_name'])){
												?>
										<span class="w--200 h--200 rounded d-inline-block bg-light bg-cover lazy"
											data-background-image="<?php echo base_url();?>assets/images/captures/<?php echo $checkin_data['capture_name'];?>">
											</span>
											<?php
										}
											else{
												?>
											<span data-initials="<?php echo $checkin_data['firstname'].' '. $checkin_data['lastname'];?>" class="sow-util-initials bg-danger-soft fs--50 m-0 w--200 h--200 rounded d-inline-flex justify-content-center align-items-center">
						...
						<div class="col-md-7 col-sm-7">
							...
							<!-- /Subtitle -->
									...
									<ul class="portfolio-detail-list list-unstyled m-0">
										...
										<li><span><i class="fa fa-link"></i>Host:</span> <a href="#"><?php echo $checkin_data['employee_firstname'].' '.$checkin_data['employee_lastname'];?></a></li>
										<li><?php 
        ...
        <div class="modal-footer">
        	...
        	<?php
        		...
        		if($checkin_data['checkin_status']=='check in'){
        		?>
        		<form id="grant_visitor_access" class="hide_after_submit" onsubmit="form_submitter('#grant_visitor_access', '<?php echo base_url();?>visitors/checkout_visitors' ); return false" >
        				<input type="hidden" name="company_id" value="<?php echo $_SESSION['admin_account']['company_id'];?>">
		        		<input type="hidden" name="visitor_tag" value="<?php echo $checkin_data['visitor_tag'];?>">
		        		<input type="hidden" name="phone_email" value="<?php echo $checkin_data['visitor_email'];?>">
		        		<input type="hidden" name="checkout_desk" value=""/>
						<input type="hidden" name="desk_id" value=""/>
		        		<input type="hidden" name="comment" value="Checkout by admin">
			            <button type="submit" class="btn btn-sm btn-pill btn-danger btn-soft">
			                <i class="fi fi-check"></i> 
			               Force Checkout
			            </button>
		        </form>
