...
class Employees extends CI_Controller
{
  ...
  //        print_r($list_employees);
	...
	$location_id = $_SESSION['company_account']['location_id'];
	  $data['list_employee_data'] = $this->Employee->search_employee($searchdata, $company_id, $location_id);
		// if (!empty($data['list_employee_data'])){ 
		foreach ($data['list_employee_data'] as $list_employee) {

			$list[] = [
				'label'		=> $list_employee["firstname"] .' '. $list_employee["lastname"].' -'.$list_employee["department"],
				'disabled'	=> false,
			];
