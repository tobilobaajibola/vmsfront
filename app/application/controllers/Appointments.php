	  ...
	  function add_appointments(){
		...
		if ($this->form_validation->run() == FALSE) {
         $data['response_msg']=validation_errors();
			...
			$host_id = get_employee_id_from_full_name($employeename, $company_id, $location_id);    
	   ...
	   function check_in_appointments(){
			...
			// compare date
			$expected_date = str_replace('-', '', $data['verify_appointment']['expected_date']);
			$today_date =  str_replace('-', '', date('Y-m-d'));
			if($expected_date >= $today_date  and $data['verify_appointment']['status'] == 0 ){
