         ...
         $data['response_msg']= validation_errors();
			...
			$password=password_verify($this->input->post('password'), $hashed_email_password);
 			...
 			if ($password >= 1){
 				...
 				$login_data =  array('admin_id' =>$data['verify_email']['id'], 'email'=>$data['verify_email']['email'], 'company_id' =>$data['verify_email']['company_id'],'firstname'=>$data['verify_email']['firstname'], 'lastname'=> $data['verify_email']['lastname'], 'level' => $data['verify_email']['level'], 'role' => $data['verify_email']['role']);
