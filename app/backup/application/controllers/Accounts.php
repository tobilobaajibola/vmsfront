		...
		if ($this->form_validation->run() == FALSE) {
         $data['response_msg']=validation_errors();
			...
			// verify with form login
 			...
 			if ($token >= 1){
 				$login_data =  array('company_id' =>$data['verify_company_code']['id'], 'id' =>$data['verify_company_code']['company_email'],'company'=>$data['verify_company_code']['company_name'], 'company_code' => $data['verify_company_code']['company_code'],'loginchannel' =>  $loginchannel, 'company_logo' => $data['verify_company_code']['company_logo'], 'company_background_image' => $data['verify_company_code']['background_image'], 'company_checkout_image' => $data['verify_company_code']['check_out_image'], 'desk_name'=>$data['verify_company_code']['desk_name'] );
