...
class Visitor extends CI_Model
{
	...
	function checkin_visitor ($checkin_data = '')
	{
		...
		$this->db->update('visitor_tag',  array('status' => 1, 'last_checkin_visitor_id' => $checkin_data['visitor_id'] ));
	...
	function checkout_visitor($checkout){
		...
		$this->db->update('checkin',  array('check_out_time' => $checkout['checkout_time'], 'check_out_date' => $checkout['checkout_date'], 'status' => $checkout['status']  ));
	    ...
	    $this->db->update('visitor_tag',  array('status' => 0));
	...
	function get_visitor_tag($company_id){
		$get_visitor_tag = $this->db->get_where('visitor_tag',  array('company_id' => $company_id, 'status' => 0 ));
    ...
    function get_list_of_checkin($company_id, $no_of_checkin)
    {
	...
	where (c.visitor_id = v.id  and v.company_id = $company_id)order by c.id desc LIMIT $no_of_checkin") ;
