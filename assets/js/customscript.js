  ...
  $(document).ready(function() {
       ...
       function checkin_visitor(){
         ...
         // $(document).ajaxSend(function() {
           ...
           $("#checkinloading").css('display', 'block');
           // });
              $.ajax({
           type: "POST",
           ...
           success:function(data){
                ...
                console.log(data);
                   ...
                   // display new triggered error
                    $("#alert_fail").css('display', 'block');
                    $('#alert_fail').append('<div>'+ data.error +'</div>');
                    $("#checkinloading").css('display', 'none');
...
function capture_trigger() {
  ...
  // $("#captureModal").on('click', take, function () {
      ...
      // remove capture div so users do not double click
      $("#capture-controls").css('display', 'none');
      // display screen loader to stop user from clicking the capture button in case of slow network
    $("#capture-loading").css('display', 'block');
        ...
        xhr.onload = function(){
          if (xhr.status==403 || xhr.status==404) {
            ...
            $("#capture-trigger").css('display', 'none');
             $(capture_input).append('<div class="alert text-success js-file-input-item d-flex clearfix shadow-md mb-2 p--2 rounded"><div class="position-relative d-inline-block bg-cover">  <img src="'+ab_url+'assets/images/captures/'+FileName+'" width="80" height="80"/></div>'+
                                    '<div class="pl--15 pr--15 flex-fill d-flex align-items-center"><span class="text-truncate d-block min-w-0 line-height-1"> Successfully captured <br><span class="fs--12 d-block pt-1" data-file-preview-img-height="50"</span></span></div></div>');
             $(capture_input).append('<input type="hidden" name="upload_image" value = "'+FileName+'"/>');
              $("#checkinbutton").css('display', 'block');
             // close the camera pop 
             
            $("#captureaccordion").css('display', 'none');
            $("#checkinvisitor").css('display', 'block');
             // $("#captureModal").hide(300);
