...
class Host extends CI_Model
{
	...
	function list_employee($company_id){
		   $get_list_employee = $this->db->get_where('host', array('company_id' => $company_id ));

			return $get_list_employee->result_array();
	}

	function get_employee_detail($employee_id, $user_company_id){
		   $get_employee_detail = $this->db->get_where('host', array('id'=>$employee_id, 'company_id' => $user_company_id ));
			return $get_employee_detail->row_array();
	}

		function search_employee($searchdata, $company_id){
		   
			$get_list_employee = $this->db->query("SELECT * from host where company_id = $company_id and  (firstname like '%$searchdata%' OR lastname like '%$searchdata%')  order by firstname, lastname ");
			
			return $get_list_employee->result_array();
	}

		function get_employee_by_name($firstname, $lastname, $company_id){
			$get_employee_by_name = $this->db->query("SELECT * from host where firstname = '$firstname' and  lastname = '$lastname' and company_id = '$company_id' ");
